...
15. `generateSceneJsonString` options: `planFirst`, `capabilityReview` (default on), `maxCapabilityReviewAttempts`, `maxTokens` (**default 6000** for generation), and optional segmented output. Segmentation is **not** the normal path: `segmentedOutput: "auto"` (default) activates it only when `estimatedSegments > 1`; use `segmentedOutput: true` to force it or `false` to guarantee a single response. `maxSceneSegments` only limits an already-active segmented run and does not activate one.
