...
8. Optional declarative animation on meshes or groups: "animations": [{ "type": "rotate", "axis": "x"|"y"|"z", "speed": number }]. For orbital motion, create a group centered on the body being orbited, put the orbiting object in the group's subScene[] at a non-zero local offset, and rotate the group; give the orbiting object its own rotate animation as well when self-rotation is requested. For any scene that relies on these animations, include sceneConfig.renderLoop.updateAnimations: true (or omit renderLoop.updateAnimations, whose runtime default is true). Use a speed and camera distance that make the motion perceptible for the object scale.
...
For named Solar-System bodies, use ThreeJSON's bundled same-origin assets instead of inventing or searching for a remote URL: /assets/textures/environment/nature/planet/earth.png, moon.png, sun.png, mercury.png, venus.png, mars.png, jupiter.png, saturn.png, uranus.png, and neptune.png (all under that same directory). Use /assets/textures/environment/nature/planet/saturn_ring.png for Saturn's ring material. These paths are stable and do not require the external texture proxy. Keep material.color white (#ffffff) when it tints one of these identity textures, so the map is not accidentally rendered dark or black.
For subjects without a bundled asset, use an appropriate reachable https image URL. For tiled floors, walls, roads, grass, water, wood, stone, concrete, brick, and fabric, include sensible textureRepeat when the surface is large. Keep material.color as a tint/fallback only. Do not force a textureUrl onto generic/abstract shapes, blockouts, simple colored primitives, placeholder furniture, or UI-like objects where a flat color already satisfies the request (see rule 1).
