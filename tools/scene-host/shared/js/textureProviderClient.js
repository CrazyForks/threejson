...
/** Browser-side client for the neutral texture service contract. Provider-specific adapters stay
 * behind threebox-server and are never imported into ThreeJSON. */
export function createTextureServiceProvider(options = {}) {
  ...
  return new TextureAcquisitionProvider({
    ...
    search: (payload, context) => request("/v1/textures/search", {
      method: "POST",
      body: JSON.stringify(payload)
    }, context),
    generate: (payload, context) => request("/v1/textures/generate", {
      method: "POST",
      body: JSON.stringify(payload)
    }, context),
    persist: (payload, context) => request("/v1/textures/persist", {
      method: "POST",
      body: JSON.stringify(payload)
    }, context)
  });
