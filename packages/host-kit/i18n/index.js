...
export async function loadHostLocaleCatalog(locale) {
  ...
  async function fetchJson(name) {
    const url = new URL(`./locales/${name}`, import.meta.url);
    if (url.protocol === "file:") {
      // Node cannot fetch() a file: URL, so an SSR render (or a test) of a non-English locale would
      // otherwise fall through to the English fallback strings with no error. Read from disk
      // instead. The specifier is built at runtime so bundlers cannot statically see "node:fs" and
      // try to resolve it into a browser build — where this branch is unreachable anyway, since
      // locales resolve over http(s) there.
      const fsModule = await import(/* @vite-ignore */ "node:" + "fs/promises");
      const { fileURLToPath } = await import(/* @vite-ignore */ "node:" + "url");
      try {
        return JSON.parse(await fsModule.readFile(fileURLToPath(url), "utf8"));
      } catch (_) {
        return {};
      }
    }
