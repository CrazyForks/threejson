/**
 * Editor settings schema — ported from tools/scene-host/shared/js/editorSettingsSchema.js.
 ...
 *
 * Two defaults differ from the tools/scene-host original: EDITOR_SETTINGS_JSON_URL now resolves
 * through sceneHostAssetUrl (CDN-first, see sceneHostPaths.js's docblock) instead of a repo-root-
 * relative new URL(); exitNavigateUrl's default no longer points at examples/html-demo/demo.html,
 * since that folder is part of the ThreeJSON monorepo, not the published @threejson/assets
...
export const EDITOR_SETTINGS_JSON_URL = sceneHostAssetUrl("json/other/scene-editor/setting.json");
...
export const EDITOR_SETTINGS_DEFAULTS = {
    ...
    general: {
      ...
      defaultSceneUrl: "json/portShow.json",
...
export const EDITOR_SETTINGS_FIELDS = [
    ...
    { section: "ai", path: "ai.texturePipelineEnabled", type: "checkbox", label: "场景显示后自动完善纹理" },
    {
      ...
      label: "纹理获取策略",
      options: [
        { value: "semantic-hybrid", label: "语义混合（推荐）" },
        { value: "manifest", label: "仅内置资源" },
        { value: "search", label: "优先搜索" },
        { value: "generate", label: "优先生图" }
      ]
    },
    ...
    { section: "ai", path: "ai.texturePbr", type: "checkbox", label: "启用完整 PBR 纹理" },
    {
      ...
      label: "纹理归档策略",
      options: [
        { value: "remote", label: "远程代理" },
        { value: "archive-selected", label: "归档已选择" },
        { value: "archive-all", label: "归档全部" }
      ]
    },
