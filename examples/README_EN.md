...
**Prerequisites**: `html-demo` needs static HTTP only; `vue-app` / `react-app` / `electron-apps` / `script` need **Node.js ≥ 24** ([`doc/en/development.md`](../doc/en/development.md)).
...
- **`html-demo/`**: static HTML + import map tutorials (read alongside `doc/`). Serve the **repository root** over HTTP; do not use `file://`.
...
See [`demo.html`](../demo.html) and [`doc/en/tutorial.md`](../doc/en/tutorial.md) for the full catalog.
