...
1. **文档同步**：修改代码必须同步更新 `doc/` 等项目文档（API、JSON 契约、示例说明、工具链 README 等），不得只改实现不留文档。
2. **方案先行**：修改代码前必须先有**方案文档**，且方案须经**人类评审**（记录评审人、日期与结论）；未经评审不得合入。
3. **提交附带方案**：提交代码时，应在 [`doc/dev/plans/`](dev/plans/) 目录中附带与该次变更相关的方案材料。
...
5. **方案文档组成**：每个方案子目录应至少包含下列三类文档（可拆为多个 `.md` 文件，见 [`doc/dev/plans/README.md`](dev/plans/README.md)）：
...
完整说明与示例见 **[`doc/dev/plans/README.md`](dev/plans/README.md)**。

## 语言与文档策略

| 层级 | 策略 |
|------|------|
| **文档 `doc/`** | 对外契约以 `doc/` + [`doc/en/`](en/) 双轨为准；新文档尽量同步英文 |
| **代码注释** | 新代码与公开 API **英文**；`core/`、`domains/` 存量中文注释 **分批英文化**（允许删除无效注释、补缺失说明；触达文件时继续迁移） |
| **[`demo.html`](../../demo.html)** | 单页运行时 i18n；`navigator.language` → `zh-CN` / `en-US`，无法识别时回退 `en-US`；中文 UI「相关文档」指向 `doc/*.md`；英文 UI 指向 `doc/en/*.md`（无英文镜像的文档仍链中文，如 `event-mechanism.md`） |
