...
Track 7 HTML pages include `troika-three-text` in the import map (SDF mode). Other tutorial pages omit it because the text dependency stays lazy until used; only a page that explicitly imports `threejson/archive` must map `fflate`.
...
5. For lessons using SDF text, keep `troika-three-text` in the page import map (see Track 7). A lesson that explicitly uses `threejson/archive` separately needs the `fflate` mapping.
