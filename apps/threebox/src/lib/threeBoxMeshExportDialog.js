/**
 * Ported from tools/scene-host/threebox/js/threeBoxMeshExportDialog.js (app-local). A singleton
 * overlay appended to document.body, independent of the React tree (same technique as the original);
 * the scene card just awaits the returned promise. Labels resolve through host-kit's t().
 */
import { t } from "@threejson/host-kit/i18n/index.js";

export const THREEBOX_MESH_EXPORT_FORMATS = Object.freeze([
...
/** Opens a ThreeBox-styled format picker and resolves to a format string or null. */
export function openThreeBoxMeshExportDialog(options = {}) {
  activeClose?.();
  return new Promise((resolve) => {
    ...
    select.value = THREEBOX_MESH_EXPORT_FORMATS.some((item) => item.value === initialFormat) ? initialFormat : "glb";
...
/** Keeps mesh-export warnings visible until the user dismisses them. */
export function showThreeBoxMeshExportWarningDialog(warnings = []) {
  activeClose?.();
  const messages = (Array.isArray(warnings) ? warnings : [warnings])
    .map((entry) => String((typeof entry === "string" ? entry : entry?.message) || "").trim())
    .filter(Boolean);
  if (!messages.length) {
